...
- `deck_7_inv.cir` — 7 `INV` stages chained `in0 -> out0 -> out1 -> ... -> out6`, driven by a single pulsed source on `in0`. Transient analysis only; dumps the waveforms to `tran_data.txt` as plain ASCII columns (`time in0`, `time out2`, `time out3`) via `wrdata`.
...
```
cd lab1
ngspice -b deck_7_inv.cir     # writes tran_data.txt (plain ASCII columns)
```
Batch mode has no interactive plot window; `tran_data.txt` is plain text (`time in0  time out2  time out3`, one row per timestep) — read it directly, with `awk`/pandas, or plot it with gnuplot (`plot 'tran_data.txt' using 1:2 with lines`).
...
- **Batch mode needs an explicit output command** — no `.print`/`.plot`/`.control ... write`/`wrdata` means ngspice exits with `no simulations run!` even if the netlist is otherwise valid. This deck uses a `.control` block with `tran` + `wrdata`.
...
- **`plot` is ignored in batch mode** (`-b`) with a harmless warning — use `wrdata` (plain ASCII columns) or `write` (binary rawfile) to dump data instead, then plot afterward from an interactive session or an external tool. `wrdata` is the more readable choice if you just want to inspect numbers.
