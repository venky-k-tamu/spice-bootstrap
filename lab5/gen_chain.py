...
The load is a single inverter F times the minimum (F = 14000 per pF, from
14000 * 71.3 aF = 1.0 pF in Lab 4 Part C; --cload picks 1 or 10 pF) and stays
fixed for every alpha. The chain has
...
Decks are named chain_<m|w>_a<alpha>.sp for 1 pF and chain_<C>pF_<m|w>_a<alpha>.sp
for other loads, with '.' written as 'p' (e.g. chain_w_a3p25.sp, chain_10pF_w_a3p25.sp).
...
Usage (from lab5/):
    ...
    python3 gen_chain.py --cload 10      # 10 pF load (F = 140000)
...
F_PER_PF = 14000  # min inverters per pF of load
# plus alpha = F (a min inverter driving the load directly), added per load
ALPHAS = [1.5, 2, 2.5, 3, 3.25, 3.5, 3.6, 3.7, 3.75, 3.8, 3.9, 4, 4.5, 5, 6, 7, 8, 9, 10]
SLEW = 5e-12  # PULSE rise/fall time (0-100%)
WN_MIN = 44e-9  # must match the WN default in inv.sub

HEADER = """\
* Lab 5 -- tapered inverter chain driving a {cload:g}pF load (modelled as INV of size F)
* sizing by {how}
...
def prefix(cload):
    return "" if cload == 1 else f"{cload:g}pF_"


def n_stages(alpha, F):
...
def sim_time(n, scale, F):
    # N=1 is a min inverter charging the load directly (~16 ns per pF). M-sized
    # chains finish their first transition well within 3 ns; W-sized ones are
    # much slower because a single wide finger carries a large gate resistance
    # (rgatemod=1, Rg ~ rshg*W/(3L)), whose gate RC grows as size^2, so their
    # window grows with the square of the load.
    r = F / F_PER_PF
    if n == 1:
        return 60e-9 * r
    return 3e-9 if scale == "m" else 20e-9 * r * r
...
def write_deck(alpha, scale, cload, out_dir, ngspice):
    F = round(F_PER_PF * cload)
    n = n_stages(alpha, F)
    last_fo = F / alpha ** (n - 1)
    tstop = sim_time(n, scale, F)
    nodes = ["in"] + [f"out{i}" for i in range(1, n + 1)]
    lines = [HEADER.format(cload=cload, how=HOW[scale], alpha=alpha, n=n, last_fo=last_fo, sub="inv.sub", F=F,
    ...
    lines.append(f"\n* {cload:g}pF load: one inverter {F}x the minimum\n"
    ...
    path = out_dir / f"chain_{prefix(cload)}{scale}_a{label(alpha)}.sp"
    path.write_text("\n".join(lines))
...
if __name__ == "__main__":
    ...
    ap.add_argument("--alphas", type=float, nargs="+", help="per-stage fanouts to generate (default: ALPHAS + [F])")
    ...
    alphas = args.alphas or ALPHAS + [round(F_PER_PF * args.cload)]
    for scale in (["m", "w"] if args.scale == "both" else [args.scale]):
        for alpha in alphas:
            path, n, last_fo = write_deck(alpha, scale, args.cload, out_dir, args.ngspice)
